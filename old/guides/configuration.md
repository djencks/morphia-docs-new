...
Morphia 2.0 introduces a number of new ways to configure its behavior.  These values are listed below:

1.  Collection naming
1.  Property naming
1.  The discriminator key
1.  The discriminator value
...
Traditionally Morphia has used with the class's "simple name" for the collection name if you choose to not manually map the name of an
 ...
 1.  `lower` This is simply the lower case form of the `identity` strategy.
 1.  `snake` This transforms element names in to their [snake case](https://en.wikipedia.org/wiki/Snake_case) versions.  For those coming
  from a Python background or who work with Python developers regularly, this should look familiar.
 1.  `camel` This transforms element names in to their [camel case](https://en.wikipedia.org/wiki/Camel_case) versions.  This is the form
  most java developers will be familiar with.
 1.  `kebab` This transforms element names in to their [kebab case](https://en.wikipedia.org/wiki/Kebab_case) versions.  This looks
...
1.  `className()` as the `DiscriminatorFunction`
1.  `identity()` as both the class and field name strategies
1.  Configures the legacy query implementation
...
1.  `simpleName()` as the discriminator function used to determine these values
1.  `camelCase()` is the default collection naming strategy
1.  `identity()` remains the property naming strategy
1.  A new `Query` implementation is configured enabling the new `Filters` based querying and disabling some older, deprecated methods.